...
诊断不会输出容器启动命令或参数，避免命令行中的订阅链接、令牌等敏感数据进入证据。
...
3. 若现有结构化能力足够，则继续操作；
4. 若需要新增技能，升级只是中间步骤；注册成功后的下一次模型调用立即获得最新系统提示与工具 Schema，并继续原目标；
...
- 新增、升级或重载技能不是任务终点；
- 每次模型调用前使用最新系统提示与 Registry 工具 Schema，使同一轮中新注册的技能下一步即可使用；
