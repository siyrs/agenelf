"""Observable self-model and controlled autonomy tools.

This capability reports runtime facts and routes improvements through the sandboxed
pipeline.  It never describes Agenelf as subjectively conscious and composes with the
persistent ``agent.self_development`` capability.
"""
...
CAPABILITY_META = {
    ...
    "name": "自我模型与自主反思",
    "description": (
        "观察自身能力、限制和运行状态，生成改进计划，并可把小型带测试补丁"
        "送入安全晋升管道；持久化沉淀由 agent.self_development 提供。"
    ),
    "version": "0.2.0",
    ...
    "operations": [
        {
            "name": "self_snapshot",
            "description": "返回当前可观测自我模型、安全不变量与持续成长摘要",
            "risk": "read",
        },
        {
            "name": "self_assess",
            "description": "检查能力缺口、加载错误与迭代状态",
            "risk": "read",
        },
        {
            "name": "autonomy_cycle",
            "description": "生成计划，或在 app-tmp 内执行一次带测试的自主改进",
            "risk": "change",
        },
        {
            "name": "autonomy_status",
            "description": "查询最近的自主循环记录",
            "risk": "read",
        },
    ],
    "composes_with": [
...
TOOLS = [
    {
        ...
        "function": {
            "name": "self_snapshot",
            "description": (
                "查看 Agenelf 当前加载的技能、能力域、运行队列、自我迭代状态、"
                "持久化成长摘要与不可违反的安全不变量。结果是软件运行状态，不代表主观意识。"
            ),
    ...
    },
    {
        ...
        "function": {
            "name": "autonomy_cycle",
            "description": (
                "执行一次受控自主循环。apply_changes=false 只生成计划；true 时只在 app-tmp "
                "生成最多四个 Python 文件的补丁，强制包含测试，测试通过后仅申请晋升。"
            ),
            "parameters": {
                "type": "object",
                "properties": {
...
def execute(tool_name: str, args: dict) -> str:
    ...
    try:
        ...
        if tool_name == "self_snapshot":
            method = getattr(agent, "self_snapshot", None)
            return _dump(method() if callable(method) else engine.snapshot())
        if tool_name == "self_assess":
            method = getattr(agent, "self_assess", None)
            return _dump(method() if callable(method) else engine.assess())
        if tool_name == "autonomy_cycle":
            method = getattr(agent, "run_autonomy_cycle", None)
            kwargs = {
                "goal": str((args or {}).get("goal", "")),
                "apply_changes": bool((args or {}).get("apply_changes", False)),
            }
            return _dump(method(**kwargs) if callable(method) else engine.run_cycle(**kwargs))
        cycle_id = str((args or {}).get("cycle_id", "")).strip()
        method = getattr(agent, "autonomy_status", None)
        if callable(method):
            return _dump(method(cycle_id))
