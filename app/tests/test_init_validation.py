...
class InitValidationTest(unittest.TestCase):
    def test_init_creates_validation_config_without_overwrite(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(status["validation"])
            target = root / "local" / "validation.yaml"
            original = target.read_text(encoding="utf-8")
            target.write_text("checks: {custom: {type: tcp, host: x, port: 1}}\nsuites: {}\n", encoding="utf-8")
