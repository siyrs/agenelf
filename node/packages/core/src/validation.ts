...
type ValidationOperation = "run_check" | "run_suite";
type ValidationState = "succeeded" | "failed" | "done" | "invalid" | "locked";
interface ValidationConfig { checks: Record<string, JsonObject>; suites: Record<string, JsonObject | JsonValue[]>; }
...
function jsonPath(value: JsonValue, dottedPath: string): { found: boolean; value: JsonValue } {
  ...
  for (const part of dottedPath.split(".")) {
    if (Array.isArray(current) && /^\d+$/.test(part)) {
      ...
      if (index < current.length) { current = current[index]; continue; }
...
async function boundedBody(response: Response): Promise<{ text: string; bytes: number; truncated: boolean }> {
  ...
  while (true) {
    ...
    if (item.value.byteLength > available) {
      if (available) chunks.push(Buffer.from(item.value.slice(0, available)));
      bytes += available;
      truncated = true;
      await reader.cancel();
      break;
...
export class ValidationQueue {
  ...
  async initialize(): Promise<void> {
    ...
    await Promise.all([mkdir(this.requests, { recursive: true }), mkdir(this.results, { recursive: true }), mkdir(this.locks, { recursive: true })]);
  }
  ...
  hasCheck(name: string): boolean { return Boolean(this.ready().checks[name]); }
  hasSuite(name: string): boolean { return Boolean(this.ready().suites[name]); }
  ...
  catalog(): JsonObject {
    ...
    return {
      checks: Object.entries(config.checks).map(([name, cfg]) => ({
        name, type: String(cfg.type ?? "unknown"), description: String(cfg.description ?? ""),
        tags: Array.isArray(cfg.tags) ? cfg.tags.map(String).slice(0, 20) : []
      })),
      suites: Object.keys(config.suites).map((name) => ({ name, checks: this.suiteMembers(name) }))
  ...
  async submit(operation: ValidationOperation, target: string, summary: string): Promise<JsonObject> {
    ...
    const id = randomId("val-");
    const request: JsonObject = {
      schema_version: 1, id, ...payload, risk: "read", summary: String(summary || "").trim(),
      fingerprint: ValidationQueue.fingerprint(payload), created_at: nowIso(), created_by: "agenelf-node-agent"
    };
...
export class ValidationRunner {
  ...
  private validateRequest(request: JsonObject): { operation: ValidationOperation; target: string } {
    ...
    if (parameters !== null && parameters !== undefined && (!asObject(parameters) || Object.keys(parameters).length)) throw new Error("验证请求不得携带自由参数");
  ...
  private async httpCheck(name: string, cfg: JsonObject): Promise<JsonObject> {
    const url = new URL(String(cfg.url ?? "").trim());
    if (url.protocol !== "http:" && url.protocol !== "https:") throw new Error(`HTTP 检查 ${name} 的 URL 非法`);
    const method = String(cfg.method ?? "GET").toUpperCase();
    if (method !== "GET" && method !== "HEAD") throw new Error(`HTTP 检查 ${name} 仅支持 GET/HEAD`);
    const expected = (Array.isArray(cfg.expected_status) ? cfg.expected_status : [cfg.expected_status ?? 200])
      .filter((item): item is number => typeof item === "number").slice(0, 20);
    if (!expected.length) expected.push(200);
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), boundedInt(cfg.timeout_seconds, 5, 1, 30) * 1_000);
    const started = performance.now();
    let statusCode: number | null = null;
    let body = { text: "", bytes: 0, truncated: false };
    let networkError = "";
    try {
      const response = await fetch(url, { method, headers: { "user-agent": "Agenelf-Validation/1.0" }, signal: controller.signal, redirect: "follow" });
      statusCode = response.status;
      body = await boundedBody(response);
    } catch (error) { networkError = `${error instanceof Error ? error.name : "Error"}: ${error instanceof Error ? error.message : String(error)}`; }
    ...
    if (equals && Object.keys(equals).length) {
      ...
      let parseError = "";
      try { parsed = JSON.parse(body.text) as JsonValue; } catch (error) { parseError = error instanceof Error ? error.message : String(error); }
    ...
    return { name, type: "http", passed: limited.every((item) => item.passed === true), latency_ms: latencyMs, observed: { status_code: statusCode, body_bytes: body.bytes, body_truncated: body.truncated }, assertions: limited };
  }
  ...
  private async tcpCheck(name: string, cfg: JsonObject): Promise<JsonObject> {
    ...
    await new Promise<void>((done) => {
      ...
      let settled = false;
      const finish = (error = "") => { if (settled) return; settled = true; errorText = error; clearTimeout(timer); socket.destroy(); done(); };
      const timer = setTimeout(() => finish("TimeoutError: TCP 连接超时"), boundedInt(cfg.timeout_seconds, 5, 1, 30) * 1_000);
  ...
  async runCheck(name: string): Promise<JsonObject> {
    ...
    try {
      const result = type === "http" ? await this.httpCheck(name, cfg) : type === "tcp" ? await this.tcpCheck(name, cfg) : (() => { throw new Error(`检查 ${name} 使用不支持的类型：${type}`); })();
      return { ...result, started_at: started, finished_at: nowIso() };
    } catch (error) {
      return { name, type: type || "unknown", passed: false, latency_ms: null, observed: {}, assertions: [assertion("configuration_or_execution", false, `${error instanceof Error ? error.name : "Error"}: ${error instanceof Error ? error.message : String(error)}`)], started_at: started, finished_at: nowIso() };
    }
  }
  async execute(request: JsonObject): Promise<JsonObject> {
    ...
    return { schema_version: 1, id: String(request.id ?? ""), capability: CAPABILITY, operation, target, status: failed === 0 && checks.length ? "succeeded" : "failed", started_at: started, finished_at: nowIso(), summary: `${passed}/${checks.length} 个检查通过，${failed} 个失败`, passed, failed, checks };
  }
  async processRequest(path: string): Promise<ValidationState> {
    ...
    if (await exists(resultPath)) return "done";
    try {
      return await withDirectoryLock(join(this.queue.locks, `${id}.lock`), async () => {
        if (await exists(resultPath)) return "done" as ValidationState;
        try {
          ...
          const result: JsonObject = { schema_version: 1, id, capability: CAPABILITY, status: "failed", reason: `${error instanceof Error ? error.name : "Error"}: ${error instanceof Error ? error.message : String(error)}`, finished_at: nowIso() };
          await atomicWriteJson(resultPath, result, true);
  ...
  async processOnce(): Promise<Record<string, number>> {
    ...
    const files = (await readdir(this.queue.requests)).filter((item) => /^val-[0-9a-f]{16}\.json$/.test(item)).sort();
