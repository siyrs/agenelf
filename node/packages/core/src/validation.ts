...
export class ValidationQueue {
  ...
  async submit(operation: ValidationOperation, target: string, summary: string): Promise<JsonObject> {
    ...
    if (operation === "run_check" && !config.checks[target]) throw new Error(`未知验证检查：${target}`);
    if (operation === "run_suite" && !config.suites[target]) throw new Error(`未知验证套件：${target}`);
  ...
  suiteMembers(name: string): string[] {
    ...
    const unknown = result.filter((item) => !this.ready().checks[item]);
...
export class ValidationRunner {
  ...
  private validateRequest(request: JsonObject): { operation: ValidationOperation; target: string } {
    ...
    const catalog = this.queue.catalog();
    const aliases = new Set((operation === "run_check" ? catalog.checks : catalog.suites) as JsonValue[]);
    void aliases;
    if (operation === "run_check" && !this.queue["config"]?.checks[target]) throw new Error(`未知验证检查：${target}`);
    if (operation === "run_suite" && !this.queue["config"]?.suites[target]) throw new Error(`未知验证套件：${target}`);
  ...
  private async tcpCheck(name: string, cfg: JsonObject): Promise<JsonObject> {
    ...
    await new Promise<void>((resolvePromise) => {
      ...
      const timer = setTimeout(() => { errorText = "TimeoutError: TCP 连接超时"; socket.destroy(); resolvePromise(); }, timeout);
      socket.once("connect", () => { clearTimeout(timer); socket.destroy(); resolvePromise(); });
      socket.once("error", (error) => { clearTimeout(timer); errorText = `${error.name}: ${error.message}`; resolvePromise(); });
    });
    const latencyMs = Math.round((performance.now() - started) * 100) / 100;
  ...
  async runCheck(name: string): Promise<JsonObject> {
    const config = this.queue["config"] as ValidationConfig | null;
    const cfg = config?.checks[name];
    if (!cfg) throw new Error(`未知验证检查：${name}`);
