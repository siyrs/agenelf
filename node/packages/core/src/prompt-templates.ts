...
export class PromptTemplateLoader {
  ...
  private async discoverDirectory(directory: string, source: "builtin" | "owner"): Promise<PromptTemplateRecord[]> {
    ...
    for (const entry of entries.sort((a, b) => a.name.localeCompare(b.name))) {
      ...
      result.push({
        ...
        path,
        body: parsed.body,
        hash: sha256({ content: parsed.body } as JsonObject)
      });
    }
    return result;
  }
  ...
  catalog(): JsonObject[] {
    return [...this.templates.values()].sort((a, b) => a.name.localeCompare(b.name)).map((template) => ({
      ...
      explicit_command: `/prompt:${template.name}`,
  ...
  commands(): string[] {
    return [...this.templates.keys()].sort().flatMap((name) => [`/${name}`, `/prompt:${name}`]);
  }
  ...
  expandCommand(line: string): JsonObject | null {
    ...
    const match = text.match(/^\/(?:prompt:)?([a-z][a-z0-9-]{0,47})(?:\s+([\s\S]*))?$/);
