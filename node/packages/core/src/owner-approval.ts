...
export class OwnerApprovalStore {
  ...
  async loadRequest(requestId: string): Promise<JsonObject> {
    const request = await readJson<JsonObject | null>(await this.requestPath(requestId), null);
    if (!request) throw new ApprovalError(`待审批请求不是合法 JSON：${requestId}`);
    return request;
  }

  private validateRequestDocument(request: JsonObject): JsonObject {
    if (request.schema_version !== 1) throw new ApprovalError("不支持的待审批请求版本");
    safeId(request.id, REQUEST_ID, "请求 ID");
  ...
  async validatePendingRequest(request: JsonObject): Promise<JsonObject> {
    this.validateRequestDocument(request);
  ...
  private async writeDecisionExact(path: string, decision: JsonObject): Promise<{ value: JsonObject; idempotent: boolean }> {
    try {
      ...
      if (
        existing && existing.decision === decision.decision && existing.fingerprint === decision.fingerprint &&
        String(existing.reason ?? "") === String(decision.reason ?? "") && String(existing.decided_by ?? "") === String(decision.decided_by ?? "")
  ...
  async applyDecision(
    ...
    const request = this.validateRequestDocument(await this.loadRequest(id));
    ...
    const decidedBy = String(options.decidedBy ?? "owner");
    const duplicates: Array<{ id: string; request: JsonObject; decision: JsonObject }> = [];
    for (const duplicateValue of options.duplicates ?? []) {
      const duplicateId = safeId(duplicateValue, REQUEST_ID, "重复请求 ID");
      if (duplicateId === id) continue;
      const duplicateRequest = this.validateRequestDocument(await this.loadRequest(duplicateId));
      ...
      duplicates.push({
        ...
        decision: {
          schema_version: 1, request_id: duplicateId, decision: "deny", fingerprint: duplicateRequest.fingerprint,
          decided_at: nowIso(), decided_by: decidedBy, reason: `superseded_by:${id}`
        }
      });
    }
    ...
    const decision: JsonObject = {
      ...
      decided_at: nowIso(), decided_by: decidedBy, reason
    };
    const primary = await this.writeDecisionExact(join(this.authDecisions, `${id}.json`), decision);
  ...
  async submitCommand(
    ...
    const command: JsonObject = {
      ...
      reason: String(options.reason ?? "").trim(), decided_by: String(options.decidedBy ?? "owner"),
      created_at: createdAt.toISOString(),
      expires_at: new Date(createdAt.getTime() + Math.max(1, Math.min(Math.trunc(options.expiresInSeconds ?? 120), 600)) * 1000).toISOString(),
      duplicates: (options.duplicates ?? []).map((item) => safeId(item, REQUEST_ID, "重复请求 ID"))
    };
  ...
  async verifyCommand(document: JsonObject, key: Buffer, at = new Date()): Promise<JsonObject> {
    ...
    const request = this.validateRequestDocument(await this.loadRequest(requestId));
    ...
    for (const duplicate of duplicates) safeId(duplicate, REQUEST_ID, "重复请求 ID");
    ...
    const actualBytes = Buffer.from(String(document.signature ?? ""), "ascii");
  ...
  async processCommand(document: JsonObject, key: Buffer): Promise<JsonObject> {
    ...
    const decision = await this.applyDecision(String(document.request_id), {
      ...
      reason: String(document.reason ?? ""), decidedBy: String(document.decided_by ?? "owner"),
...
export class ApprovalRunner {
  ...
  async processOnce(): Promise<Record<string, number>> {
    ...
    for (const name of names.filter((item) => /^apc-[0-9a-f]{16}\.json$/.test(item))) {
      const path = join(this.store.commands, name);
      const info = await lstat(path).catch(() => null);
      if (info?.isFile() && !info.isSymbolicLink()) candidates.push({ name, mtime: (await stat(path)).mtimeMs });
    }
