...
export class OwnerApprovalStore {
  ...
  private async normalizeDuplicates(request: JsonObject, values: string[]): Promise<string[]> {
    const ids = [...new Set(values.map((item) => safeId(item, REQUEST_ID, "重复请求 ID")))].filter((item) => item !== request.id);
    if (ids.length > MAX_DUPLICATES) throw new ApprovalError(`重复请求超过 ${MAX_DUPLICATES} 个上限`);
    for (const id of ids) {
      const duplicate = await this.validatePendingRequest(await this.loadRequest(id));
  ...
  async submitCommand(
    ...
    const command: JsonObject = {
      ...
      duplicates: await this.normalizeDuplicates(request, options.duplicates ?? [])
    };
  ...
  async verifyCommand(document: JsonObject, key: Buffer, at = new Date()): Promise<JsonObject> {
    ...
    commandString(document.decided_by ?? "owner", "decided_by", 200);
    ...
    await this.normalizeDuplicates(request, duplicates.map(String));
    return request;
  }
