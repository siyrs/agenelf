...
interface SourceLine { indent: number; text: string; line: number; }
...
function stripComment(raw: string): string {
  ...
  for (let index = 0; index < raw.length; index += 1) {
    ...
    else if (!single && !double && (char === "]" || char === "}")) depth = Math.max(0, depth - 1);
    else if (char === "#" && !single && !double && depth === 0 && (index === 0 || /\s/.test(previous))) return raw.slice(0, index).trimEnd();
  }
  return raw.trimEnd();
}
...
function splitMapping(text: string, line: number): [string, string] {
  ...
  for (let index = 0; index < text.length; index += 1) {
    ...
    else if (char === ":" && !single && !double && depth === 0) {
      const key = text.slice(0, index).trim();
      if (!key || key === "<<") throw new Error(`YAML 第 ${line} 行包含非法 key`);
...
function splitInline(value: string): string[] {
  ...
  for (let index = 0; index < value.length; index += 1) {
    ...
    if (char === "," && !single && !double && depth === 0) { items.push(current.trim()); current = ""; }
    else current += char;
  }
...
function parseScalar(raw: string, line: number): JsonValue {
  ...
  if (value.startsWith("{") && value.endsWith("}")) {
    try {
      ...
      return parsed;
    } catch { throw new Error(`YAML 第 ${line} 行内联 object 仅支持合法 JSON`); }
  }
...
function parseBlock(lines: SourceLine[], start: number, indent: number, depth: number): { value: JsonValue; next: number } {
  ...
  if (arrayMode) {
    ...
    while (index < lines.length && lines[index].indent === indent) {
      ...
      if (!rest) {
        ...
        result.push(child.value); index = child.next; continue;
      }
      if (rest.includes(":")) {
        ...
        if (rawValue) object[key] = parseScalar(rawValue, source.line);
          ...
          const child = parseBlock(lines, index + 1, lines[index + 1].indent, depth + 1);
          object[key] = child.value; index = child.next - 1;
        } else object[key] = {};
        if (index + 1 < lines.length && lines[index + 1].indent > indent) {
          ...
          if (!continuation.value || Array.isArray(continuation.value) || typeof continuation.value !== "object") throw new Error(`YAML 第 ${source.line} 行数组 object 的续行必须是 mapping`);
          Object.assign(object, continuation.value); index = continuation.next - 1;
        }
        result.push(object);
  ...
  while (index < lines.length && lines[index].indent === indent) {
    ...
    if (rawValue) { result[key] = parseScalar(rawValue, source.line); index += 1; continue; }
    if (index + 1 >= lines.length || lines[index + 1].indent <= indent) { result[key] = {}; index += 1; continue; }
    const child = parseBlock(lines, index + 1, lines[index + 1].indent, depth + 1);
    result[key] = child.value; index = child.next;
  }
  return { value: result, next: index };
}
...
export function parseSimpleYaml(text: string): JsonObject {
  ...
  if (parsed.next !== lines.length || !parsed.value || Array.isArray(parsed.value) || typeof parsed.value !== "object") throw new Error("YAML 顶层必须是 mapping");
  return parsed.value;
}
