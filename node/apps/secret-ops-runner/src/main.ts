...
import { SecretOpsRunner } from "../../../packages/core/src/secret-ops.ts";

function intervalMs(): number {
  const value = Number(process.env.AGENELF_SECRET_OPS_INTERVAL_MS ?? 500);
  return Math.max(200, Math.min(Number.isFinite(value) ? value : 500, 60_000));
}

async function heartbeat(root: string, counts: Record<string, number>, status = "ok"): Promise<void> {
  await atomicWriteJson(resolve(root, "data", "runner-health", "secret-ops-runner.json"), {
    schema_version: 1,
    name: "secret-ops-runner",
    runtime: "node-typescript",
    role: "owner-secret-env",
    status,
    pid: process.pid,
...
export async function runOnce(root = process.env.AGENELF_ROOT || process.cwd()): Promise<Record<string, number>> {
  ...
  const runner = new SecretOpsRunner(resolved);
