...
import { ChangeOpsRunner } from "../../../packages/core/src/change-ops.ts";

function intervalMs(): number {
  const value = Number(process.env.AGENELF_CHANGE_OPS_INTERVAL_MS ?? 500);
...
async function heartbeat(root: string, counts: Record<string, number>, status = "ok"): Promise<void> {
  await atomicWriteJson(resolve(root, "data", "runner-health", "ops-runner.json"), {
    schema_version: 1,
    name: "ops-runner",
    runtime: "node-typescript",
    role: "change-privileged",
    status,
    pid: process.pid,
...
export async function runOnce(root = process.env.AGENELF_ROOT || process.cwd()): Promise<Record<string, number>> {
  ...
  const runner = new ChangeOpsRunner(resolved);
