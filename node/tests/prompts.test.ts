...
test("prompt templates discover slash commands and expand arguments", async () => {
...
test("owner prompt overrides builtin without exposing file paths or code execution", async () => {
  const value = await root();
...
test("prompt frontmatter rejects unsupported metadata", async () => {
  const value = await root();
